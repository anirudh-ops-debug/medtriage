...
import { Activity, Bed, Users, Stethoscope, HeartPulse, Syringe, AlertTriangle } from "lucide-react";

const StatCard = ({ label, value, sub, icon: Icon, alert }: { label: string; value: string | number; sub?: string; icon: any; alert?: boolean }) => (
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats((prev) => ({
        ...
        icuBeds: Math.max(0, Math.min(8, prev.icuBeds + (Math.random() > 0.7 ? -1 : Math.random() > 0.5 ? 1 : 0))),
      ...
      <div className="animate-fade-up">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Real-time hospital overview</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          ...
          <StatCard label="ICU Beds Free" value={stats.icuBeds} sub="of 8 total" icon={Bed} alert={stats.icuBeds < 3} />
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          ...
          <StatCard label="Nurses On Duty" value={stats.nurses} sub="Shift B active" icon={Users} />
          <StatCard label="OT Active" value={`${stats.otStatus}/5`} sub="2 scheduled" icon={Syringe} />
