                    ...
                    <div key={o.id} className="p-3 rounded-lg border border-border bg-secondary">
                      ...
                      {editIdx === i && editData ? (
                        <div className="space-y-2 mt-2 pt-2 border-t border-border">
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              <label className="text-[9px] text-muted-foreground">{t("organs.donor")}</label>
                            ...
                            <div>
                              <label className="text-[9px] text-muted-foreground">{t("organs.status")}</label>
                          ...
                          <div className="flex gap-2">
                            <button onClick={handleSave} className="flex items-center gap-1 px-3 py-1 rounded bg-primary text-primary-foreground text-xs font-semibold"><Check size={12} /> {t("organs.save")}</button>
                            <button onClick={() => { setEditIdx(null); setEditData(null); }} className="flex items-center gap-1 px-3 py-1 rounded bg-secondary border border-border text-xs text-muted-foreground"><X size={12} /> {t("common.cancel")}</button>
                          </div>
                        </div>
                      ) : (
                        <div className="text-[10px] text-muted-foreground">
                          {o.donor_details || t("organs.noDonorInfo")} · {t("organs.blood")}: {o.blood_type || t("organs.na")}
                        </div>
                      )}
                    </div>
                  ))}
            ...
            {selectedPatient ? (
              <div className="stat-card space-y-3">
                <div className="flex items-center justify-between">
                  <h2 className="text-xs font-semibold text-foreground">{t("organs.patientDetails")}</h2>
                  <button onClick={() => navigate(`/patients/${selectedPatient.id}`)} className="text-[10px] text-primary hover:underline">{t("organs.fullProfile")}</button>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px]">
                    <div><span className="text-muted-foreground">{t("common.name")}:</span> <span className="text-foreground font-semibold">{selectedPatient.name}</span></div>
                    <div><span className="text-muted-foreground">{t("register.age")}:</span> <span className="text-foreground">{selectedPatient.age}{selectedPatient.gender}</span></div>
                    <div><span className="text-muted-foreground">{t("common.admitted")}:</span> <span className="text-foreground">{selectedPatient.admissionDate}</span></div>
                    <div><span className="text-muted-foreground">{t("organs.status")}:</span> <span className="text-foreground font-semibold">{selectedPatient.riskLevel}</span></div>
                  </div>
                  {selectedPatient.diagnosis && (
                    <div className="p-2 rounded bg-primary/5 border border-primary/20">
                      <p className="text-[9px] text-muted-foreground">{t("patientList.diagnosis")}</p>
                ...
                <div className="pt-2 border-t border-border flex items-center gap-2">
                  ...
                  <span className="text-[9px] text-muted-foreground">{t("organs.thoaCompliance")}</span>
                  <CheckCircle size={10} className="text-medical-green" />
                </div>
              </div>
            ) : (
              <div className="stat-card flex items-center justify-center h-48">
                <p className="text-xs text-muted-foreground">{t("organs.selectPatient")}</p>
              </div>
            )}
          </div>
        </div>
